  ...
  return (
    <div className=' w-full h-[370px] bg-white rounded-lg shadow-sm p-3 relative cursor-pointer'>
        ...
        <Link to={`/product/${product_name}`}>
          ...
          className=' w-full h-[170px] object-contain' alt=''/>
        </Link>
        <Link to="/">
        ...
        <Link to={`/products/${product_name}`}>
          ...
          <div className='flex'>
            ...
            <div className='py-2 flex items-center justify-between'>
              <div className='flex '>
                <h5 className={`${styles.productDiscountPrice}`}>${data.price===0 ? data.price : data.discount_price}</h5>
                <h4 className={`${styles.price}`}>
                  {data.price ? data.price + "$" : null}
                </h4>
              </div>
              <span className='font-[400] text-[17px] text-green-400'>{data.total_sell} Sold</span>
            </div>
        ...
        <div>
        {
         ...
         <AiOutlineEye size={22} className=' cursor-pointer absolute right-2 top-14' onClick={()=>setOpen(!open)} color='#333'
         title='Quick View'/>
         <AiOutlineShoppingCart size={25} className=' cursor-pointer absolute right-2 top-24' onClick={()=>setOpen(!open)}
         color='#444'
         title='Add to Cart'
         />
         {
